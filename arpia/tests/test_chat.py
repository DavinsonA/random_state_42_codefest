...
from src.tools import corpus  # noqa: E402  (registra `search_corpus` en el registry)
...
class FakeGraph:
    ...
    def invoke(self, state: dict, config: dict | None = None) -> dict:
        ...
        registry.get("search_corpus")(query=state["question"])
        for _ in range(2):
            usage.record_usage(
...
def test_chat_devuelve_el_formato_de_adl(stub_client):
    ...
    assert body["evaluacion"]["tools_called"][0]["name"] == "search_corpus"
...
def test_evaluacion_refleja_solo_el_turno_actual(live):
    ...
    assert [t["name"] for t in tools] == ["search_corpus"]
